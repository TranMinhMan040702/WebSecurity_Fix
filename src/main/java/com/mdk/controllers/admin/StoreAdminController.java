...
@WebServlet(urlPatterns = { "/admin/store/all", "/admin/store/statistic", "/admin/store/all/delete-soft",
		"/admin/store/all/delete", "/admin/store/all/restore" })
public class StoreAdminController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (url.contains("admin/store/all/delete-soft")) {
			...
			for (Store item : reportStore)
				item.setAvatar(userService.findById(item.getOwnerID()).getLastname() + " "
						+ userService.findById(item.getOwnerID()).getFirstname());
			...
			while (arrEachMonth.size() < 12) {
				...
				for (Store item : storeList)
					if (Integer.parseInt(item.getCreatedAt().toString().substring(5, 7)) == count
							&& Integer.parseInt(item.getCreatedAt().toString().substring(0, 4)) == localDate.getYear())
						totalEachMonth++;
					else if (Integer.parseInt(item.getCreatedAt().toString().substring(5, 7)) == count && Integer
							.parseInt(item.getCreatedAt().toString().substring(0, 4)) == (localDate.getYear() - 1))
	...
	protected void sendEmail(HttpServletRequest req, HttpServletResponse resp, String storeId, String state)
