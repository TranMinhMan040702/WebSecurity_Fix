							...
							<div class="col-lg-12">
								<c:if test="${empty listCart}">
									<div class="iq-card pt-5 pb-4">
										<div class="col-lg-12 d-flex justify-content-center">
											<img style="width: 20%" src="<c:url value=" /template/images/emptycart.png" />" />
										</div>
										<div class="col-lg-12 text-center mb-5">
											<span class="text-second text-uppercase font-size-20">Giỏ
												hàng của bạn đang trống. <a href="<c:url value=" /web/book/search" />">Cùng mua sắm nào!</a>
											</span>
										</div>
									</div>
								</c:if>
								<c:forEach items="${listCart}" var="cart">
									<div class="iq-card">
													...
													<li class="checkout-product">
														<form method="post">
															<div class="row align-items-center">
																<div class="col-sm-1">
																	<span class="col-12 p-0 position-relative image-overlap-shadow" style="height: 50px;">
																		...
																		<div class="col-sm-10">
																			<div class="row align-items-center mt-2">
																						...
																						<c:choose>
																							<c:when test="${cartItem.product.quantity > 0}">
																								<input name="count" class="form-control"
																									style="max-width: 75px; height: 35px" type="number"
																									min="${-1*cartItem.count+1}" max="${cartItem.product.quantity}"
																									step="1" value="1">
																							</c:when>
																							<c:otherwise>
																				...
																				<div class="col-sm-5 col-md-6">
																					<span
																						class="product-price">${cartItem.product.promotionalPrice*cartItem.count}</span>
																					<c:set var="cartTotal"
										...
										<button id="place-order" class="btn btn-primary d-block mt-3 next ">
												<a href="<c:url value="/web/cart/delivery?cart=${cart.id}" />">Thanh toán</a>
												</button>
										</div>
									</div>
								</c:forEach>
